...
export default function DiagnosisHistoryVisit({ medicalRecord }) {
  return (
    <>
      <h3 className="text-xl font-semibold text-teal-500">تشخیص پزشک</h3>
      {medicalRecord.filter((e) => e.typeId === 3).length > 0 &&
        medicalRecord
          .filter((e) => e.typeId === 3)
          .map((e, i) => (
            <div key={e.id} className="px-3">
              <div className="flex flex-wrap items-center rounded-lg shadow-md duration-300 hover:shadow-lg hover:bg-slate-50 mt-2 p-2">
